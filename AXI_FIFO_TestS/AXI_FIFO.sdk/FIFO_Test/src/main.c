...
float speed_tx;
float speed_rx;
static XGpio Led;//LED显示传输或接受状态
static XGpio pCount;//通过FPGA部分进行数据包个数统计，使用一个GPIO的输入发送到PS部分
XGpio Gpio;
...
/*
 * 统计全局定时器当前计数值，计算相对于初始化全局定时器后过了多长时间。
 *
 */
double toc(void)
{
 ...
 * 统计数据传输个数。
 *
 * @param	InstancePtr 指向DMA对象的指针
 * @param	BuffAddr 数据基地址
 * @param	Direction 数据传输方向
 * @return
 * 		此次DMA传输的数据长度，单位为字节。实际是读取DMA状态寄存器的值
 */
...
int axi_dma_test()
{
	...
	//for(i = 0; i < Tries; i ++)
	{
		...
		if(RX_ready)
		{
			...
			RX_ready=0;
			//XAxiDma_SimpleTransfer，根据参数修改DMA控制寄存器的值
			//参数分别为DMA指针，数据传输首地址，数据长度，数据传输方向
		...
		if(usec==10)//约5秒，若设置时间过长会导致全局定时器溢出，数据不准确
		{
			usec=0;
			time = toc();
			time = time / 1000;//toc获取的时间单位为ms
			//统计上次中断满10次到现在这段时间间隔内“创造”的数据包个数。
			cnt=count;
			count = XGpio_DiscreteRead(&pCount, 1);
			sprintf(oled_str,"count=%d",count - cnt);
			xil_printf("%s\r\n",oled_str);
			//输出成功接受数据包个数，数据字节数，传输速率
...
void init_intr_sys()
{
	DMA_Intr_Init(&AxiDma0Rx,XPAR_AXIDMA_0_DEVICE_ID);//初始化DMA
	DMA_Intr_Init(&AxiDma1Tx,XPAR_AXIDMA_1_DEVICE_ID);
	Timer_init(&Timer,TIMER_LOAD_VALUE,0);//初始化私有定时器
	Init_Intr_System(&Intc); // 初始化中断
	Setup_Intr_Exception(&Intc);
	DMA_Setup_RX_Intr_System(&Intc,&AxiDma0Rx,RX_INTR_ID);//注册DMA中断
	DMA_Setup_TX_Intr_System(&Intc,&AxiDma1Tx,TX_INTR_ID);
	Timer_Setup_Intr_System(&Intc,&Timer,TIMER_IRPT_INTR);//注册定时器中断
	DMA_Intr_Enable(&Intc,&AxiDma0Rx,0);//DMA中断使能
